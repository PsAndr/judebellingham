...
public class ExpressionParser implements TripleParser {
    ...
    public TripleExpression parse(final String expression) {
        ...
        if (ans == null) {
            throw new WrongExpressionException(expression);
        }
        if (ans.rBound < expression.length() - 1) {
            throw new WrongExpressionException(expression);
        }
            ...
            switch (token) {
                ...
                case VARIABLE:
                    if (!enableVariables.contains(tokenVal.value.substring(tokenVal.value.length() - 1))) {
                        throw new WrongExpressionException("Wrong variable name: " + tokenVal.value);
                    }
                    prevPart = new ParsePart(new Variable(tokenVal.value), tokenVal.rightBound);
                    continue;
                case CONST:
                    if (prevPart != null) {
                        throw new WrongExpressionException("Unexpected constant value: " + tokenVal.value);
                    }
