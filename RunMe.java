...
public final class RunMe {
	...
	long t = ((target >> 32) << 32);
	long t1 = t;
	long t2 = ((t ^ (t >>> 32)) ^ target) & tt;
	t |= t2;
	System.out.println(target);
	System.out.println(t ^ (t >>> 32));
	// print(4, ((target >> 32) << 32), password);
        for (long i = t1; i < Long.MAX_VALUE; i++) {
            if ((i ^ (i >>> 32)) == target) {
                print(4, i, password);
		return;
            }
    ...
    private static void flag13(final byte[] password, final long depth, long result) {
        for (long i = depth; i < MAX_DEPTH; i++) {
            result = (result ^ PRIME) | (result << 2) + i * 17;
        }
